...
class record:
    def __init__(self,benchmark:str,max_WSS:int) -> None:
        self.benchmark=benchmark
        self.max_WSS=max_WSS
...
def load_history()->list:
    ...
    with open("experiment_history","r") as f:
        ...
        for line in raw_lines:
            ...
            max_WSS=int(raw_data[1])
            result.append(record(benchmark,max_WSS))
    return result

def save_history(result:list)->list:
    with open("experiment_history","w") as f:
        for i in result:
            f.write(f"{i.benchmark},{i.max_WSS}\n")
...
def test_recording_thread(benchmark:str,data:dict,interval:int, recording_thread_stop_event):
    while not recording_thread_stop_event.is_set():
        ...
        data['max_WSS']=max(data['max_WSS'],rss_count)
        set_rss_count(0)
        time.sleep(1)
...
def run_one_time(benchmark:str):
    ...
    max_WSS=tmp_data['max_WSS']
    results.append(record(benchmark=benchmark,max_WSS=max_WSS))
    data={
        'benchmark':benchmark,
        'max_WSS':max_WSS
    }
    log_utils.data_upload("max_WSS",data)
