...
assert "spec-optimized-with-scan-interval" in kernelname
...
class record:
    def __init__(self,scan_interval:int,runtime:float) -> None:
...
def load_history()->list:
    ...
    with open("experiment_history","r") as f:
        ...
        for line in raw_lines:
            ...
            result.append(record(scan_interval,benchmarkRuntime))
    return result

def save_history(result:list)->list:
    with open("experiment_history","w") as f:
        for i in result:
            f.write(f"{i.scan_interval},{i.runtime}\n")

spec_utils.set_path("/root/cpu2006")

results=load_history()

def run_one_time(scan_interval:int):
    global results
    ...
    results.append(record(scan_interval=scan_interval,runtime=benchmark_runtime))
    data={
        'scan_interval':scan_interval,
        'runtime':benchmark_runtime
    }
    log_utils.data_upload("spec_optimized_with_scan_interval", data)
...
for scan_interval in scan_intervals:
    skip_flag=False
    for i in results:
        if i.scan_interval==scan_interval:
            skip_flag=True
            break
    if skip_flag:
        continue
    run_one_time(scan_interval)
