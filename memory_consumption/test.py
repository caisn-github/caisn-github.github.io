...
assert "page-consumption" in kernelname
...
DEBUGFS_PAGE_CONSUMPTION_FILE=f"{DEBUGFS_DIR}/page_consumption"
...
def get_page_consumption()->int:
    return udebugfs.read_from_debugfs_u64(DEBUGFS_PAGE_CONSUMPTION_FILE)
...
class record:
    def __init__(self,benchmark:str,page_consumption:float) -> None:
        self.benchmark=benchmark
        self.page_consumption=page_consumption
...
def load_history()->list:
    ...
    with open("experiment_history","r") as f:
        ...
        for line in raw_lines:
            ...
            page_consumption=float(raw_data[1])
            result.append(record(benchmark,page_consumption))
    return result

def save_history(result:list)->list:
    with open("experiment_history","w") as f:
        for i in result:
            f.write(f"{i.benchmark},{i.page_consumption}\n")
...
def run_one_time(benchmark:str):
    ...
    clean_scan()
    # stop_stress()
    page_consumption=get_page_consumption()
    spec_utils.clean_one_benchmark(benchmark)
    results.append(record(benchmark=benchmark,page_consumption=page_consumption))
    data={
        'benchmark':benchmark,
        'page_consumption':page_consumption
    }
    log_utils.data_upload("page_consumption", data)
