...
assert "insert-time" in kernelname
...
DEBUGFS_INSERT_TIME_SUMMARY_FILE=f"{DEBUGFS_DIR}/insert_time_summary"
DEBUGFS_INSERT_TIME_AMOUNT_FILE=f"{DEBUGFS_DIR}/insert_time_amount"
...
def get_insert_time_summary()->int:
    return udebugfs.read_from_debugfs_u64(DEBUGFS_INSERT_TIME_SUMMARY_FILE)

def set_insert_time_summary(summary:int)->None:
    udebugfs.write_to_debugfs_u64(DEBUGFS_INSERT_TIME_SUMMARY_FILE,summary)

def get_insert_time_amount()->int:
    return udebugfs.read_from_debugfs_u64(DEBUGFS_INSERT_TIME_AMOUNT_FILE)

def set_insert_time_amount(amount:int)->None:
    udebugfs.write_to_debugfs_u64(DEBUGFS_INSERT_TIME_AMOUNT_FILE,amount)
...
class record:
    def __init__(self,benchmark:str,insert_time:float) -> None:
...
def load_history()->list:
    ...
    with open("experiment_history","r") as f:
        ...
        for line in raw_lines:
            ...
            insert_time=float(raw_data[1])
            result.append(record(benchmark,insert_time))
    return result

def save_history(result:list)->list:
    with open("experiment_history","w") as f:
        for i in result:
            f.write(f"{i.benchmark},{i.insert_time}\n")
...
def run_one_time(benchmark:str):
    ...
    set_insert_time_summary(0)
    set_insert_time_amount(0)
    ...
    clean_scan()
    # stop_stress()
    insert_time_summary=get_insert_time_summary()
    insert_time_amount=get_insert_time_amount()
    insert_time=insert_time_summary/insert_time_amount
    spec_utils.clean_one_benchmark(benchmark)
    results.append(record(benchmark=benchmark,insert_time=insert_time))
    data={
        'benchmark':benchmark,
        'insert_time':insert_time
    }
    log_utils.data_upload("insert_time", data)
