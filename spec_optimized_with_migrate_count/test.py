...
assert "spec-optimized-with-migrate-count" in kernelname
...
class record:
    def __init__(self,migrate_max_count:int,runtime:float) -> None:
        self.migrate_max_count=migrate_max_count
...
def load_history()->list:
    ...
    with open("experiment_history","r") as f:
        ...
        for line in raw_lines:
            ...
            if len(raw_data)<2:
                continue
            migrate_max_count=int(raw_data[0])
            benchmarkRuntime=float(raw_data[1])
            result.append(record(migrate_max_count,benchmarkRuntime))
    return result

def save_history(result:list)->list:
    with open("experiment_history","w") as f:
        for i in result:
            f.write(f"{i.migrate_max_count},{i.runtime}\n")
...
def run_one_time(migrate_max_count:int):
    global results
    ...
    time.sleep(1)
    enable_migrate()
    set_migrate_max_count(migrate_max_count)
    ...
    results.append(record(migrate_max_count=migrate_max_count,runtime=benchmark_runtime))
    data={
        'migrate_max_count':migrate_max_count,
        'runtime':benchmark_runtime
    }
    log_utils.data_upload("spec_optimized_with_migrate_count", data)
    save_history(results)
    os.system("reboot")
    time.sleep(10)

set_nvm()
migrate_max_counts=[i for i in range(200,2200,200)]
for migrate_max_count in migrate_max_counts:
    skip_flag=False
    for i in results:
        if i.migrate_max_count==migrate_max_count:
            skip_flag=True
            break
    if skip_flag:
        continue
    run_one_time(migrate_max_count)
